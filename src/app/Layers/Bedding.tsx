...
const Bedding: React.FC<BeddingProps> = ({ selectedLayer }) => {
  ...
  useEffect(() => {
    ...
    const layoutRadius =
      coreLayerRadius.length > 0
        ...
        : insulatedRadius + 50;
        const lastCorePos = finalCorePositions[finalCorePositions.length - 1];
const coreDistance = Math.sqrt(lastCorePos.x ** 2 + lastCorePos.y ** 2);

// Total outer radius: distance to furthest core + insulation + thickness + wire radius
    ...
    finalCorePositions.forEach((corePos, coreIndex) => {
      ...
      if (isCompactedFinal) {
        ...
        svg
          ...
          .attr("cx", centerX)
          .attr("cy", centerY)
          .attr("r", beddingOuterRadius+1.25)
          .attr("fill", "none")
